...
- 首页更多设置新增 ProMotion实验模式 beta，默认关闭，用于对比B站弹幕和王者侧边吸附掉帧问题
- 实验模式开启后，ContentSource 画中画会额外启用静默视频保活，并按设备最高帧率生成占位视频，尝试用视频路径维持全局高刷
- 实验模式下时间悬浮窗改为被动帧率检测并降低文字刷新频率，减少对前台App渲染的干扰；需重新打开悬浮窗后完整生效
- 新增国际化基础，主界面、使用教程、帧率演示、版本页、更新日志和常见问题开始支持英文系统显示
- 新增英文系统 App 名称 `Global Refresh`，中文系统仍显示 `全局高刷`
- 保留中文诊断日志，方便继续排查悬浮窗、后台保活、发热和帧率问题
- 从 1.0.8 fix2 稳定源码锚点继续开发
